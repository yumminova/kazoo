...
exec_cmd(_Node, _UUID, JObj, _ControlPid, _DestId) ->
    lager:notice("command ~s not meant for us but for ~s", [kz_json:get_value(<<"Application-Name">>, JObj), _DestId]),
...
get_fs_app(Node, UUID, JObj, <<"page">>) ->
    ...
    case kapi_dialplan:page_v(JObj) of
        ...
        'true' ->
            [EP0 | _] = Endpoints,
            AccountId = kz_json:get_value([<<"Custom-Channel-Vars">>, <<"Account-ID">>], EP0),
            PageId = <<"page_", AccountId/binary, "_", (kz_binary:rand_hex(8))/binary>>,
